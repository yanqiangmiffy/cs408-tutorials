...
看到这些公式头大是正常的。树这一块最忌死背，最好办法是先在脑子里画图。可以把树想成家族族谱，或者公司的组织架构。

#### 考点 1：结点数 = 总度数 + 1

- **大白话**：结点的“度”就是它往下伸了几根树枝，所有结点度数加起来，其实就是整棵树的总边数。
- **为什么还要加 1**：除了根结点以外，每个结点都恰好有一条边从上面连向它，所以边数 = 结点数 - 1。
- **结论**：总结点数 = 总度数 + 1。

#### 考点 2：“度为 m 的树” 和 “m 叉树” 不是一回事

- **度为 m 的树**：这是“至少有一个结点真的有 m 个孩子”。
- **m 叉树**：这是“任何结点最多只有 m 个孩子”。
- **区别关键词**：
  - “度为 m”强调**必须出现**
  - “m 叉树”强调**不能超过**
...
因为从根到第 `i` 层，一共把“最多生 m 个孩子”这件事连续用掉了 `i-1` 次。

#### 考点 4：高度为 h 的 m 叉树最多有多少个结点

如果每层都塞满，总结点数就是每层最大结点数相加：
...
要让结点数尽量少，树就应该尽量长得像“一根单链条”。

- **高度为 h 的 m 叉树最少结点数**
  - 只要求高度是 `h`，并没有要求谁必须有 `m` 个孩子。
  - 那么每层放 1 个结点就够了。
  - **结论**：最少 `h` 个结点。

- **高度为 h、度为 m 的树最少结点数**
  - 这里除了高度，还额外要求“必须至少有一个结点的度等于 m”。
  - 为了省结点，只让某一层恰好出现一次“生满 m 个孩子”，其余层继续单链条。
  - **结论**：`(m + 1) + (h - 2) = h + m - 1`

#### 考点 6：具有 n 个结点的 m 叉树最小高度

如果固定有 `n` 个结点，想让高度最小，就必须让树尽量“胖”，也就是每层尽量塞满。

于是高度 `h` 必须满足：

`n <= (m^h - 1) / (m - 1)`

把它反过来解 `h`，再向上取整，就得到最小高度：

`⌈log_m(n(m - 1) + 1)⌉`

#### 学习建议

遇到这类题，先别背公式。先画一棵 `m=2` 或 `m=3`、高度为 `3` 的小树，把每层结点数写出来，再去看公式，基本都会自己长出来。
