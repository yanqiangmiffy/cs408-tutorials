...
- 如果把数组每个位置都用满，那么“空”和“满”都可能出现 `front == rear`，状态冲突。
- 最直接的做法就是主动空出一个单元。
- 这样一来：
  - `front == rear` 一定是空
  - `rear` 再前进一步撞上 `front` 才算满
...
- 调用顺序：`factorial(3) → factorial(2) → factorial(1)`，不断入栈
- 返回顺序：`factorial(1) → factorial(2) → factorial(3)`，不断出栈
- 所以递归的返回顺序天然符合栈的 **后进先出**
...
| 题目 | 训练点 |
|------|--------|
| [05 用两个栈实现队列](/coding-interview-offer/05) | 栈与队列互相模拟 |
| [20 包含min函数的栈](/coding-interview-offer/20) | 辅助栈 |
| [21 栈的压入、弹出序列](/coding-interview-offer/21) | 栈序列合法性 |
